...
export default function RecentAlerts({ alerts }) {
  return (
    <div className="space-y-3">
      {alerts.map((alert) => {
        const isClean = alert.severity === 'info' || alert.severity === 'clean' || alert.severity === 'good';
        ...
        return (
          <div key={alert.id} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors group">
            <div className={`mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
              isCritical ? 'bg-red-100' : isClean ? 'bg-emerald-100' : 'bg-amber-100'
            }`}>
              {isClean ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-1">
                ...
                <Badge variant={isClean ? 'clean' : isCritical ? 'critical' : 'moderate'} size="xs">
                  {isCritical ? 'Critical' : isClean ? 'Clean Air' : 'Moderate'}
                </Badge>
              </div>
