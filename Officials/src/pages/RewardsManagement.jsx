...
function RewardModal({ reward, onClose, onSaved, official }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${API}/api/rewards/leaderboard-eligible?barangay=${encodeURIComponent(form.barangay)}`,
      )
      .then(({ data }) => {
    ...
    try {
      if (isEdit) {
        const { data } = await axios.patch(`${API}/api/rewards/${reward._id}`, {
          ...
          notes: form.notes,
          claimDeadline: form.claimDeadline,
          ...(publish
            ...
            {searchQ.length >= 2 &&
              ...
              !form.recipientId && (
                <p className="text-xs text-slate-400 mt-1.5 px-1">
                  No residents found for "{searchQ}" in Brgy.{" "}
                  {form.barangay || "this barangay"}.
                </p>
              )}
          </div>
...
function DetailModal({ reward, onClose, onUpdated, official }) {
  ...
  const [currentSig, setCurrentSig] = useState(
    reward.officialSignatureUrl || null,
  );
  ...
  const saveSignature = async () => {
    ...
    try {
      ...
      const { data: up } = await axios.post(`${API}/api/upload`, {
        data: dataUrl,
      });
      const { data: updated } = await axios.patch(
        `${API}/api/rewards/${reward._id}`,
        { officialSignatureUrl: up.url },
      );
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[1000] flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        {/* Status bar */}
        <div
          className={`h-1.5 w-full ${reward.status === "claimed" ? "bg-emerald-500" : reward.status === "published" ? "bg-blue-500" : reward.status === "expired" ? "bg-red-400" : "bg-slate-300"}`}
        />
        ...
        <div className="p-5 border-b border-slate-100 flex items-start justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-xl hover:bg-slate-100 text-slate-400 flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="bg-slate-50 rounded-xl p-3 space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-slate-400 text-xs">Barangay</span>
              <span className="font-semibold text-slate-800">
                {reward.barangay}
              </span>
            </div>
            ...
            {reward.claimCode && (
              <div className="flex justify-between items-center">
                <span className="text-slate-400 text-xs">Claim Code</span>
                <span className="font-mono font-bold text-emerald-700 text-xs bg-emerald-50 px-2 py-0.5 rounded-lg">
                  {reward.claimCode}
                </span>
              </div>
            )}
            {reward.claimDeadline && (
              <div className="flex justify-between">
                ...
                <span
                  className={`font-semibold text-xs ${days != null && days <= 7 ? "text-red-600" : "text-slate-700"}`}
                >
                  {new Date(reward.claimDeadline).toLocaleDateString()}{" "}
                  {days != null
            ...
            {reward.claimedDate && (
              <div className="flex justify-between">
                <span className="text-slate-400 text-xs">Claimed On</span>
              ...
              {currentSig && !showSigPad && (
                <div className="p-3 bg-white">
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    {reward.issuedByName || official?.name}
                  </p>
                </div>
              )}
                ...
                <div className="p-3 bg-white space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={clearCanvas}
                      disabled={!hasDrawn}
                      className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200 disabled:opacity-40"
                    >
                      <Trash2 className="w-3 h-3" /> Clear
                    </button>
                    <button
                      onClick={saveSignature}
                      disabled={!hasDrawn || sigSaving}
                      className="flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-bold text-white bg-emerald-700 rounded-lg hover:bg-emerald-800 disabled:opacity-40"
                    >
                      {sigSaving ? (
                        <>
          ...
          <div className="flex items-center gap-1 pt-1">
            {["draft", "published", "claimed"].map((s, i) => (
              <div key={s} className="flex items-center gap-1 flex-1">
                <div
                  className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${["draft", "published", "claimed"].indexOf(reward.status) >= i ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-400"}`}
                >
                  {i + 1}
                </div>
                ...
                {i < 2 && (
                  <div
                    className={`flex-1 h-0.5 ${["draft", "published", "claimed"].indexOf(reward.status) > i ? "bg-emerald-500" : "bg-slate-200"}`}
                  />
                )}
              </div>
            ))}
        ...
        <div className="px-5 pb-5 space-y-2">
          ...
          {(reward.status === "draft" || reward.status === "published") &&
            !showRevoke && (
              <button
          ...
          {showRevoke && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 space-y-2">
              <p className="text-xs text-red-700 font-semibold">
                Revoke within 24 hours of publishing only. This cannot be
                undone.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowRevoke(false)}
                  className="flex-1 py-1.5 text-xs font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl"
                >
                  Cancel
                </button>
                <button
                  onClick={revoke}
                  disabled={loading}
                  className="flex-1 py-1.5 text-xs font-semibold text-white bg-red-600 hover:bg-red-700 rounded-xl disabled:opacity-50"
                >
                  Confirm Revoke
                </button>
              </div>
...
export default function RewardsManagement() {
  ...
  const fetchRewards = useCallback(async () => {
    ...
    try {
      ...
      if (official?.barangay && official.barangay !== "All")
        params.set("barangay", official.barangay);
  ...
  const onUpdated = (reward) => {
    if (!reward) {
      ...
      setRewards((prev) =>
        prev.map((r) => (r._id === reward._id ? reward : r)),
      );
    }
    setDetailReward(null);
  };
  ...
  const expiringSoon = rewards.filter(
    (r) =>
      r.status === "published" &&
      daysLeft(r.claimDeadline) != null &&
      daysLeft(r.claimDeadline) <= 7,
  ).length;
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pb-4 grid grid-cols-3 gap-3">
        ...
        <div
          className={`border rounded-2xl p-4 shadow-sm ${expiringSoon > 0 ? "bg-amber-50 border-amber-200" : "bg-white border-slate-100"}`}
        >
          <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mb-1">
            Expiring Soon
          </p>
          <p
            className={`text-2xl font-extrabold ${expiringSoon > 0 ? "text-amber-600" : "text-slate-900"}`}
          >
            {expiringSoon}
          </p>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {r.claimDeadline ? (
                          <span
                            className={`text-xs font-semibold ${days != null && days <= 7 && r.status === "published" ? "text-red-600" : "text-slate-500"}`}
                          >
