...
import { 
  Search, Route, MapPin, Truck, 
  Trash2, ChevronRight, Filter, 
  Plus, MoreVertical, Map as MapIcon,
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Routes</p>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Assigned</p>
            <p className="text-2xl font-black text-slate-900">{routes.filter(r => r.truckId).length}</p>
          </div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Total Stops</p>
              ...
              {['All', 'Assigned', 'Unassigned'].map(t => (
                <button
                  ...
                  className={`px-3 py-1.5 text-[10px] font-black rounded-lg transition-all ${
                    filter === t ? 'bg-emerald-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  {t}
                </button>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-sm font-bold text-slate-900">No Routes Found</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-5">
              {filtered.map(route => (
                <div 
                  key={route._id}
                  onClick={() => navigate('/route-builder', { state: { editRoute: route } })}
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1.5">
                        <Calendar className="w-3 h-3" /> {new Date(route.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <button 
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100/50">
                      <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-1">Stops</p>
                    ...
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100/50">
                      <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-1">Truck ID</p>
                      <p className={`text-sm font-black flex items-center gap-1.5 ${route.truckId ? 'text-blue-700' : 'text-slate-400 italic'}`}>
      ...
      <div className="bg-emerald-900 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 shadow-xl border border-emerald-800">
        ...
        <div className="flex-1 text-center md:text-left">
          <h4 className="text-base font-black text-white">Live Route Tracking</h4>
