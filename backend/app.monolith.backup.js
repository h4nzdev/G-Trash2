...
function generateIoTAlerts(reading) {
  ...
  // Normal/Clean air does not produce alerts to prevent spamming notifications
...
app.post("/api/iot/sensor-data", async (req, res) => {
  ...
  try {
    ...
    // 4. AI analysis — generate alerts if thresholds exceeded (with cooldown/deduplication)
    const sensorAlertCooldowns = global._sensorAlertCooldowns || (global._sensorAlertCooldowns = new Map());
    const lastAlertInfo = sensorAlertCooldowns.get(sensorId);
    const now = Date.now();

    const alertDefs = generateIoTAlerts(reading);
    const savedAlerts = [];
    for (const a of alertDefs) {
      const isNewSeverity = !lastAlertInfo || lastAlertInfo.severity !== a.severity;
      const isCooldownPassed = !lastAlertInfo || (now - lastAlertInfo.timestamp > 15 * 60 * 1000); // 15 min cooldown

      if (isNewSeverity || isCooldownPassed) {
        ...
        sensorAlertCooldowns.set(sensorId, { severity: a.severity, timestamp: now });
      }
    }
    // If air quality returned to clean, reset cooldown state so future spike triggers immediately
    if (alertDefs.length === 0 && lastAlertInfo) {
      sensorAlertCooldowns.delete(sensorId);
    }

    // 5. Auto-create a report when air quality is Critical / Hazardous / Unhealthy (deduplicated)
    ...
    if (isCriticalOrHazardous) {
      const existingOpenReport = await Report.findOne({
        ...
        status: { $in: ["Pending", "In Progress", "In Review", "pending", "in_progress", "in_review"] },
      });
